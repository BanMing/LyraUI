...
TSharedRef<SWidget> UUIExtensionPointWidget::RebuildWidget()
{
	if (!IsDesignTime() && ExtensionPointTag.IsValid())
	{
		...
		FDelegateHandle Handle = GetOwningLocalPlayer<UGameUILocalPlayer>()->CallAndRegister_OnPlayerStateSet(
			UGameUILocalPlayer::FPlayerStateSetDelegate::FDelegate::CreateUObject(this, &UUIExtensionPointWidget::RegisterExtensionPointForPlayerState)
		);
	}
...
void UUIExtensionPointWidget::RegisterExtensionPointForPlayerState(UGameUILocalPlayer* LocalPlayer, APlayerState* PlayerState)
{
	if (UUIExtensionSubsystem* ExtensionSubsystem = GetWorld()->GetSubsystem<UUIExtensionSubsystem>())
	{
		TArray<UClass*> AllowedDataClasses;
