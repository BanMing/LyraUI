...
UPrimaryGameLayout* UGameUIPolicy::GetRootLayout(const UGameUILocalPlayer* LocalPlayer) const
{
	const FRootViewportLayoutInfo* LayoutInfo = RootViewportLayouts.FindByKey(LocalPlayer);
	return LayoutInfo ? LayoutInfo->RootLayout : nullptr;
}

void UGameUIPolicy::NotifyPlayerAdded(UGameUILocalPlayer* LocalPlayer)
{
	LocalPlayer->OnPlayerControllerSet.AddWeakLambda(this,
		[this](UGameUILocalPlayer* LocalPlayer, APlayerController* PlayerController)
		{
...
void UGameUIPolicy::NotifyPlayerRemoved(UGameUILocalPlayer* LocalPlayer)
{
	if (FRootViewportLayoutInfo* LayoutInfo = RootViewportLayouts.FindByKey(LocalPlayer))
	{
...
void UGameUIPolicy::NotifyPlayerDestroyed(UGameUILocalPlayer* LocalPlayer)
{
	NotifyPlayerRemoved(LocalPlayer);
...
void UGameUIPolicy::AddLayoutToViewport(UGameUILocalPlayer* LocalPlayer, UPrimaryGameLayout* Layout)
{
	UE_LOG(LogGameCommonUIBase, Log, TEXT("[%s] is adding player [%s]'s root layout [%s] to the viewport"), *GetName(), *GetNameSafe(LocalPlayer), *GetNameSafe(Layout));
...
void UGameUIPolicy::RemoveLayoutFromViewport(UGameUILocalPlayer* LocalPlayer, UPrimaryGameLayout* Layout)
{
	TWeakPtr<SWidget> LayoutSlateWidget = Layout->GetCachedWidget();
	if (LayoutSlateWidget.IsValid())
	{
		UE_LOG(LogGameCommonUIBase, Log, TEXT("[%s] is removing player [%s]'s root layout [%s] from the viewport"), *GetName(), *GetNameSafe(LocalPlayer), *GetNameSafe(Layout));

		Layout->RemoveFromParent();
		if (LayoutSlateWidget.IsValid())
		{
			UE_LOG(LogGameCommonUIBase, Log, TEXT("Player [%s]'s root layout [%s] has been removed from the viewport, but other references to its underlying Slate widget still exist. Noting in case we leak it."),
...
void UGameUIPolicy::OnRootLayoutAddedToViewport(UGameUILocalPlayer* LocalPlayer, UPrimaryGameLayout* Layout)
{
#if WITH_EDITOR
...
void UGameUIPolicy::OnRootLayoutRemovedFromViewport(UGameUILocalPlayer* LocalPlayer, UPrimaryGameLayout* Layout)
{
}

void UGameUIPolicy::OnRootLayoutReleased(UGameUILocalPlayer* LocalPlayer, UPrimaryGameLayout* Layout)
{
}
...
void UGameUIPolicy::CreateLayoutWidget(UGameUILocalPlayer* LocalPlayer)
{
...
TSubclassOf<UPrimaryGameLayout> UGameUIPolicy::GetLayoutWidgetClass(UGameUILocalPlayer* LocalPlayer)
{
